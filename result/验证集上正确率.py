...
def make_acc(model_type, task_type):
    loss = pd.read_csv(os.path.join(dataset , task_type, model_type, 'val_acc.csv'))
    for attr in attr_list:
        ...
        plt.ylabel('acc')
        plt.savefig(os.path.join(dataset+'/res_img/acc', "%s-%s-%s.png" % (model_type, task_type, attr)))
        plt.show()
