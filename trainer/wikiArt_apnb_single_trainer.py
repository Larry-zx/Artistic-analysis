...
from model.onepic_cnn_pool import apnb_cnn_pool
from model.apnb_vit_pool import apnb_vit_pool
from model.apnb_swin_pool import apnb_swin_pool
...
import time
import numpy as np
from data.apicNboxes import data_balance
import tqdm
...
class Classifier_Trainer(object):
    ...
    def __init__(self):
        ...
        # 负责调节学习率
        self.scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=self.optimer, T_mult=2, T_0=4)
    ...
    def set_model(self, model_type):
        ...
        print("属性:%s" % (cfg.ATTR))
        if model_type == 'apnb_cnn_pool':
            self.model = apnb_cnn_pool(class_num=cfg.class_num)
        elif model_type == 'apnb_vit_pool':
            self.model = apnb_vit_pool()
        elif model_type == 'apnb_swin_pool':
            self.model = apnb_swin_pool()
    ...
    def set_input(self, datas):
        ...
        return label, images, adj, boxes, size
    ...
    def train(self, epoch_id):
        ...
        for batch_idx, datas in tqdm.tqdm(enumerate(self.train_dataloader)):
            label, images, adj, boxes, size = self.set_input(datas)
            ...
            if (batch_idx + 1) % (len(self.train_dataloader) // 3) == 0:
                # 终端输出
                print("Epoch: %d/%d , batch_idx:%d , time:%s , Loss:%.4f " % (
