...
from model.detectionGCN_v3 import SoftPoolingGcnEncoder
import torch
...
class vit_zx_multi(BaseNetwork):
    def __init__(self, class_num=1, k_neighbour=21, gcn_mode='diffpool', pretrain=True):
        super(vit_zx_multi, self).__init__()
        self.k = k_neighbour
    ...
    def set_gcn(self, gcn_mode):
        if gcn_mode == 'diffpool':
            self.gcn = SoftPoolingGcnEncoder(max_num_nodes=self.k, input_dim=768,
                                             hidden_dim=648,
