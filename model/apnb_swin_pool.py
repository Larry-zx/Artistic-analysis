...
class SwinTransformer(BaseNetwork):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.head_zx = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
...
class apnb_swin_pool(BaseNetwork):
    def __init__(self):
        super(apnb_swin_pool, self).__init__()
        self.vit = swin_base_patch4_window7_224()
        self.GCN = SoftPoolingGcnEncoder(input_dim=1024, hidden_dim=512, embedding_dim=512,
    ...
    def vit_load_weights(self):
        ...
        self.vit.load_state_dict(torch.load(os.path.join('checkpoints/finetun/swin.pth'), map_location=device),
                                 strict=False)
        print("checkpoints/finetun/swin.pth记载完成")
        ...
        for b in range(B):
            ...
            for patch in path_id_list[:-1]:
                ...
                for id in patch:
                    id = int(id)
                    # 一个patch的特征
                    tmp_feature = vit_features[b:b + 1, id:id + 1, :]
